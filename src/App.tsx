...
const App: React.FC = () => {
  ...
  const [blockedRotate, setBlockedRotate] = React.useState(false);
  ...
  const move = ({ keyCode, repeat }: { keyCode: number; repeat: boolean }): void => {
    if (!gameOver) {
      if (keyCode === 37) {
        ...
        if (repeat) return;
        setDroptime(30);
      } else if (keyCode === 32 && !blockedRotate) {
        setBlockedRotate(true);
        playerRotate(stage);
      }
    }
  };
